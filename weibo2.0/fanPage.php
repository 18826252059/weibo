...
$sql2="select f.fid,f.time,u.username from fan as f join user u on f.fid=u.id where f.uid={$uid} {$where} order by f.time desc";
...
$sql="select f.fid,f.time,u.username from fan as f join user u on f.fid=u.id where f.uid={$uid} {$where} order by f.time desc limit {$offset},{$pageSize}";
        ...
        <div class="left_area">
            <div class="mess" style="margin-top: -10px;">
                <ul>
                    ...
                    <li><a href=""><strong><?php echo $rows['fanNum'];?></strong>粉丝</a></li>
        ...
        <div class="right_area">
            <div class="list_top">
                ...
                <div class="list_top_right">
                    <div class="navbar-form navbar-left" style="margin-top: -10px;" onkeydown="searchFanByEnter()">
                        <div class="form-group">

                            <input id="searchFan" type="text" class="form-control" placeholder="<?php if($myid==$uid){
                                echo "搜一搜我的粉丝";
                            }else{
                                if($rows['sex'] == '男'){echo "搜一搜他的粉丝";}
                                else if($rows['sex'] == '女'){echo "搜一搜她的粉丝";}
                            }?>
                            " style="width: 200px;">

                        </div>
                        <input type="button" class="btn btn-primary" onclick="searchFan()" value="搜索">
                    </div>
                </div>
            </div>
            <div class="showList">
                <?php if($keywords!=null){?>
                    <div class="item" style="float: left;margin-left: 20px;" >搜索结果:共<span ><?php echo $totalRows;?></span>个&nbsp;<a href="fanPage.php?uid=<?php echo $uid;?>">返回全部粉丝</a></div>
                <?php }else{?>
                    <div class="item" style="float: left;margin-left: 20px;" >粉丝数:共<span ><?php echo $totalRows;?></span>个</div>
                <?php }?>
            ...
            <?php foreach($rows2 as $row2):?>
                <div class="showList">
                    <?php
                    $sql="select p.id,p.username,p.sex,p.face,c.followNum,c.fanNum,c.blogNum from user as p join user_information c on p.id=c.uid  where c.uid=".$row2['fid'];
                        ...
                        <div  class="content">
                            ...
                            <?php if($row4['id']!=$myid){
                                ...
                                if(!$flag){
                                    ...
                                    ?><input type="button"  class="btn-info" style="float: right;margin-right: 20px;width: 100px;" value="＋关注" onclick="follow(<?php echo $row4['id'];?>)">
                                <?php }
                                ?>
                                <a href="myHomePage.php?uid=<?php echo $row4['id'];?>"><input type="button" class="btn-info" style="float: right;margin-right: 20px;width: 100px;" value="去Ta的主页"></a>
                            <?php  }else{?>
                                <a href="myHomePage.php?uid=<?php echo $row4['id'];?>"><input type="button" class="btn-info" style="float: right;margin-right: 20px;width: 100px;" value="去我的主页"></a>
                            <?php }?>
                        </div>
...
<script type="text/javascript">
    function searchFanByEnter(){
        if ( event.keyCode == 13){
            searchFan();
        }
    }
    function searchFan(){
        var val=document.getElementById("searchFan").value.trim();
        if(val!=""){
            window.location="fanPage.php?uid=<?php echo $uid;?>&keywords="+val;
        }
    }
