...
$where=$keywords?"and text like '%{$keywords}%'":null;
$sql2="select id,time,text,collectNum,tranNum,discussNum,approveNum from blog  where uid={$uid} {$where}";
...
$sql2="select id,time,text,collectNum,tranNum,discussNum,approveNum,isTran,tranBlogId from blog  where uid={$uid} {$where} order by time desc limit {$offset},{$pageSize}";
$rows2=fetchAll($link,$sql2);
        ...
        <div class="right_area">
            <div class="list_top">
                <div class="list_top_left">
                    <span class="both"><a href="myHomePage.php?uid=<?php echo $uid;?>">全部</a></span>
                    <!--<span class="more"><a href="">更多</a></span>-->
                </div>
                <div class="list_top_right"  >
                   <!-- <input type="text" class="search_wb" placeholder="搜索微博">-->
                    <div class="navbar-form navbar-left" style="margin-top: -10px;" onkeydown="searchBlogByEnter()">
                        <div class="form-group">

                            <input id="searchBlog" type="text" class="form-control" placeholder="<?php if($myid==$uid){
                                echo "搜一搜我的微博";
                            }else{
                                if($rows['sex'] == '男'){echo "搜一搜他的微博";}
                                else if($rows['sex'] == '女'){echo "搜一搜她的微博";}
                            }?>
                            " style="width: 200px;">

                        </div>
                        <input type="button" class="btn btn-primary" onclick="searchBlog()" value="搜索">
                    </div>
                </div>
            </div>
            <div class="showList">
                <!--<div class="item" style="float: left;">微博数:--><?php /*echo $num;*/?>
                <!--</div>-->
                <?php if($keywords!=null){?>
                    <div class="item" style="float: left;margin-left: 20px;" >搜索结果:共<span id="blognum"><?php echo $totalRows;?></span>条&nbsp;<a href="myHomePage.php?uid=<?php echo $uid;?>">返回全部微博</a></div>
                <?php }else{?>
                    <div class="item" style="float: left;margin-left: 20px;" >微博数:共<span id="blogcolnum"><?php echo $totalRows;?></span>条</div>
                <?php }?>
            ...
            <div class="showList">
                <div class="item">
                    <div class="portrait">
                        <a href="#"><img src="uploads/<?php echo $rows['face'];?>" ></a>
                    </div>
                    <div class="content">
                        <div>
                ...
                <div class="operation">
                    <ul>
                        ...
                        if($res){
                            ?>
                            <li><a style="color: red;" id="colbid-<?php echo $row2['id'];?>" onclick="colblog(<?php echo  $myid;?>,<?php echo $row2['id'];?>)" href="javascript:void(0)"><span class="glyphicon glyphicon-plus" style="color:#00a1d8" data-toggle="tooltip" title="撤销收藏"></span>(<?php echo $row2['collectNum'];?>)</a></li>
                        <?php }else{?>
                            <li><a id="colbid-<?php echo $row2['id'];?>" onclick="colblog(<?php echo  $myid;?>,<?php echo $row2['id'];?>)" href="javascript:void(0)"><span class="glyphicon glyphicon-plus" data-toggle="tooltip" title="收藏"></span>(<?php echo $row2['collectNum'];?>)</a></li>
                        <?php }?>
                        <li><a href="discussPage.php?uid=<?php echo $uid;?>&blogId=<?php echo $row2['id']?>#disArea"><span class="glyphicon glyphicon-share-alt" data-toggle="tooltip" title="转发"></span>(<?php echo $row2['tranNum'];?>)</a></li>
                        <li><a href="discussPage.php?uid=<?php echo $uid;?>&blogId=<?php echo $row2['id']?>"><span class="glyphicon glyphicon-comment" data-toggle="tooltip" title="评论"></span>(<?php echo $row2['discussNum'];?>)</a></li>
                        <?php
                        $sql="select * from approveblog where uid={$myid} and blogId={$row2['id']}";
                        $res=fetchOne($link,$sql);
                        if($res){
                            ?>
                            <li style="border-right: none;"><a style="color: red;" id="appbid-<?php echo $row2['id'];?>" onclick="appblog(<?php echo  $myid;?>,<?php echo $row2['id'];?>)" href="javascript:void(0)" ><span class="glyphicon glyphicon-heart" style="color:red;" data-toggle="tooltip" title="撤销喜欢"></span>(<?php echo $row2['approveNum'];?>)</a></li>
                        <?php }else{?>
                            <li style="border-right: none;"><a id="appbid-<?php echo $row2['id'];?>" onclick="appblog(<?php echo  $myid;?>,<?php echo $row2['id'];?>)" href="javascript:void(0)" ><span class="glyphicon glyphicon-heart" data-toggle="tooltip" title="喜欢"></span>(<?php echo $row2['approveNum'];?>)</a></li>
                        <?php }?>
...
<script >
    ...
    function searchBlogByEnter(){
        if ( event.keyCode == 13){
            searchBlog();
        }
    }
    function searchBlog(){
        var val=document.getElementById("searchBlog").value.trim();
        if(val!=""){
            window.location="myHomePage.php?uid=<?php echo $uid;?>&keywords="+val;
        }
    }
